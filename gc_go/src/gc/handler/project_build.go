...
func HandleBuildingDetail(w http.ResponseWriter,r * http.Request) {//authentic user login 
	...
	switch (r.Method) {
		...
		case "POST": 
			...
			// image_url := r.Form["image_url"][0]
			// docType:= r.Form["type"][0]
			queryStr = `insert into project_status(`+keys+`create_time) Values (`+values+`now())`
			method = "添加成功"
			break 
		case "PUT": 
			r.ParseMultipartForm(1024)
			...
			content := r.Form["content"][0]
			id := r.Form["id"][0]
			queryStr = `update  project_status set title= '`+title+`',summary='`+summary+`',content='`+content+`' Where id = `+id+``
			method = "修改成功"
			break 
		case "DELETE": 
			r.ParseMultipartForm(1024)
			ids := r.Form["ids"][0]
			queryStr = `Delete From project_status Where id in (`+ids+`)`
			method = "删除成功"
			break 
		default:break 
	}
